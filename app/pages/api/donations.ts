...
// API Response Types
type PostResponseData = { donation: Donation };
type GetResponseData = { donations: Donation[] };
...
export default async function handler(
  ...
  try {
    if (req.method === 'POST') {
      ...
      // Simulate donation processing with a promise
      processDonation(newDonation);
...
async function processDonation(donation: Donation) {
  ...
  setTimeout(() => {
    ...
    if (index !== -1) {
      donations[index].status = Math.random() > 0.1 ? 'completed' : 'failed'; // Simulating a random outcome
    }
  }, 2000);
}
