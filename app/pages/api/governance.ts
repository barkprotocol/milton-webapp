...
const ProposalSchema = z.object({
  ...
  options: z.array(z.string().min(1, "Option cannot be empty").max(100, "Option cannot exceed 100 characters"))
  ...
  try {
    ...
    if (req.method === 'POST') {
      if (action === 'propose') {
        ...
        return res.status(200).json({ message: `Proposal execution status: ${proposal.status}` });
      } else {
